...
//				dispatchEvent(new Event(Event.ACTIVATE));
			...
			if(libraryManager != null){
				if(!this.ngListManager.loadNgList(LibraryManager.instance.systemFileDir)){
					this.ngListManager.loadNgList(LibraryManager.instance.libraryDir);
				}
			}
...
//					destructor();
		...
		public function getOldCommentFromNico(date:Date):void{
			...
			if(renewDownloadManagerForOldComment != null){
				...
				if(isStreamingPlay){
					videoName = "nndd.flv";
					videoPath = LibraryManager.instance.tempDir;
				}else{
...
//		}
		...
		private function commentTimerHandler(event:TimerEvent):void{
			...
			if(this.windowType == PlayerController.WINDOW_TYPE_SWF){
				// SWFのタイムラインヘッダ、コメントタイマー時間の更新と終了監視
				var isSwfFinish:Boolean = updateSwfTimelineHeadAndCheckEnd();			
				if(isSwfFinish){
					return;
				}
			}
			...
			if(videoPlayer.videoInfoView.checkbox_SyncComment.selected 
				&& videoPlayer.videoInfoView.visible){
		...
		private function updateSwfTimelineHeadAndCheckEnd():Boolean{
...
//				this.videoInfoView.saveStore();
		...
		public function saveNgList():void{
			this.ngListManager.saveNgList(LibraryManager.instance.systemFileDir);
		}
		...
		public function reloadLocalComment(date:Date = null):void{
			...
			if(this.videoInfoView.ngListProvider != null){
				if(!this.isStreamingPlay && this.source != null && this.source != "" ){
					...
					comments = new Comments(PathMaker.createNomalCommentPathByVideoPath(LibraryManager.instance.tempDir.url + "/nndd.flv"), 
						PathMaker.createOwnerCommentPathByVideoPath(LibraryManager.instance.tempDir.url + "/nndd.flv"), 
...
//							videoPlayer.videoInfoView.owner_text_nico = ThumbInfoUtil.encodeThumbInfo(description);
		...
		private function setLocalIchibaInfo(ichibaInfoPath:String, isStreaming:Boolean):void{
			...
			try{
				...
				var html:String = fileIO.loadHtml(ichibaInfoPath);
...
//				this.streamingRetryCount = 0;
				...
				if(url.indexOf("http://") == -1){
					...
					if(videoTitle.indexOf(".swf") != -1 || videoTitle.indexOf(".SWF") != -1){
						...
						if(playList != null && playListIndex != -1){
							this.initWithPlayList(url, PlayerController.WINDOW_TYPE_SWF, comments, playList, videoNameList, playListName, playListIndex, true, true, libraryManager.tempDir.url + "/nndd.flv", videoTitle);
						}else{
							this.isPlayListingPlay = false;
						...
						if(playList != null && playListIndex != -1){
							this.initWithPlayList(url, PlayerController.WINDOW_TYPE_FLV, comments, playList, videoNameList, playListName, playListIndex, true, true, libraryManager.tempDir.url + "/nndd.flv", videoTitle);
						}else{
							this.isPlayListingPlay = false;
